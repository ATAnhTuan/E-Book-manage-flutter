...
class VietJetAPICall {
  bool? isHttps;

  VietJetAPICall() {
    if (VIETJET_URL.startsWith("https")) {
      this.isHttps = true;
    } else {
      this.isHttps = false;
    }
  ...
  Future<http.Response> postMethodNew(String endPoint,  Map? data, {requireToken = false}) async {
    var url = this._getOAuthURL("POST", endPoint);
    print(url);
   ...
   var headers = {
    ...
    if (requireToken) {
        var header = {HttpHeaders.authorizationHeader :'Bearer ${getStringAsync(TOKEN)}'};
        headers.addAll(header);
    }
    var client = new http.Client();
    var response;
        response = await client.post(Uri.parse(url), body: jsonEncode(data), headers: headers); 
    return response;
  }


    Future<http.Response> getMethodNew(String endpoint, {requireToken = false}) async {
    var url = this._getOAuthURL("GET", endpoint);
    print(url);
  ...
  Future<http.Response> deleteMethod(String endpoint, {requireToken = false}) async {
  ...
  Future<http.Response> putMethodNew(String endPoint,  Map? data, {requireToken = false}) async {
   ...
   var headers = {
    ...
    final  response = await http.put(Uri.parse(url), body: jsonEncode(data), headers: headers); 
    return response;
  }
