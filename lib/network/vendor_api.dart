...
class VendorAPI {
  ...
  Future<http.Response> putAsync(String endPoint, {Map? data, requireToken = false}) async {
    ...
    var client = new http.Client();
    var response = await client.put(Uri.parse(url), body: jsonEncode(data), headers: headers);
  ...
  Future<http.Response> deleteAsync(String endPoint, {requireToken = false}) async {
